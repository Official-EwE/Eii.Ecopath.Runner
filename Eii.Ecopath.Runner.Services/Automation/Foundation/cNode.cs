...
public class cNode
{
    ...
    private static bool TryBindAutomationArgument(object? supplied, Type targetType, out object? bound, out int score)
    {
        ...
        // Array conversion. JSON arrays are typically deserialized as object[], so
        // convert each element recursively to the requested CLR element type.
        if (targetType.IsArray && supplied is Array suppliedArray)
        {
            Type? elementType = targetType.GetElementType();
            if (elementType == null)
                return false;

            Array convertedArray = Array.CreateInstance(elementType, suppliedArray.Length);
            int arrayScore = 2;

            for (int i = 0; i < suppliedArray.Length; i++)
            {
                if (!TryBindAutomationArgument(
                        suppliedArray.GetValue(i),
                        elementType,
                        out object? convertedItem,
                        out int elementScore))
                {
                    return false;
                }

                convertedArray.SetValue(convertedItem, i);
                arrayScore += elementScore;
            }

            bound = convertedArray;
            score = arrayScore;
            return true;
        }

        // Last resort for endpoint arguments and other CLR scalar values.
